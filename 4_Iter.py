...
2. 现有迭代器用法：
    所有list/dict/tuple都是iterable可迭代对象(实现了__iter__)，但不是迭代器(没有实现__next__)
    迭代器函数： range(), enumerate(), iter(), reversed()
    最常用方法1(for循环取出迭代值)，最常用方法2(next取出迭代值)
    
3. 创建新迭代器：
    只要定义了__iter__()和__next__()方法，就是一个迭代器
'''
# --------迭代对象---------------
