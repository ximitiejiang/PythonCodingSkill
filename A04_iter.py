...
(3)生成器generator
        也就是迭代器，只是更容易创建，同样节省内存，且不需要手动去实现__iter__()和__next__()
...
"""案例2：简化生成迭代器的方式，只需要在__iter__()方法中返回一个迭代对象就可以了。
理解：
    1. __iter__()目的是创建迭代器，所以需要return一个迭代器对象。
        - 要么返回自己，此时自己需要再实现__next__来成为完整的迭代器。
        - 要么返回一个手动生成的迭代器，比如iter()，此时就不需要再写__next__了，因为返回的对象自己已经有__next__方法了。
    2. __next__()目的是自动提取迭代器的下一个值。
"""
class IterData0():
    def __init__(self, num):
        self.num = num
