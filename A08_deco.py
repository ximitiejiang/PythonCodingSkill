...
装饰器需要做如下2件事：
    1. 接收原函数名和原函数参数
    2. 执行原函数(有可能要返回原函数返回值)

方式A: 采用外函数和内函数组成一个装饰器(@就是执行装饰器，也就相当于执行原函数)
    1. 外函数接收函数名，内函数接收参数
    2. 内函数执行函数，返回原函数返回值，额外需要外函数返回内函数(内函数作为wrapper)

方式B: 采用类的方式生成一个装饰器类(@就是执行类创建对象，也就相当于执行原函数)
    1. 类的__init__接收函数名，类的__call__函数接收参数
    2. 类的__call__函数执行函数，返回原函数返回值

方式C: 采用单个函数作为一个装饰器(@就是执行装饰器，由于没有接收原类初始化参数，此时相当于原类不做任何操作返回原类名)
    1. 该单函数仅能接收类名
    2. 该单函数做完自己的操作，原封不懂返回这个类名
    
所以可以认为，装饰器本质是一个以函数或者类名作为形参，中间可做操作可不做操作，最后再返回执行的函数或返回原类名 
"""
...
核心理解3：装饰器任务是高阶函数，对函数进行二次加工，所以整个设计过程是：
            ...
            4. 最后返回wrapper
核心理解4：装饰器的两层def的功能，分别是外层获得原函数名，内层获得原函数参数，并在内层把原函数名和原函数参数组合
并且合并放在wrapper函数中，并返回wrapper函数名，也就相当于把原函数名称替换成了wrapper函数名称，而这个wrapper函数名称
之所以能够替代原函数名称，是因为只要这个wrapper函数名被作为函数执行，他内部就会调用原函数带参执行。
另一个关键是：之所以内层能够获得外层的原函数名，是因为内层wrapper函数是一个闭包函数，他在内层没有找到func变量就会
转而去外层寻找该func，并持久保存这个func变量，而不会因为函数作用域问题报错说找不到func变量。这是python的闭包函数语法，
他支持任何内层的函数到外层去寻找需要的变量名，只要找到就能拿来用(但只能使用不能修改，参考A06对闭包的解释)
...
0. 核心：
    ...
    >外层的return wrapper必不可少，否则函数消失了；
     内层的return可以没有，取决原始函数是否需要返回值 (***)
...
参考：https://blog.csdn.net/zhangfh1990/article/details/73442447 (介绍用装饰器写register注册器的写法)

1. 装饰器本身：本质是一个可以接收参数的函数，所以可以是函数，也可以是带__call__的类，也可以是对象的方法
2. 装饰器在@deco这一步运行时，就已经运行，并准备好了相关返回的wrapper
    注意：这里有一个大坑，到底装饰器作为register使用时，是什么时候运行，是什么时候能够把所有注册对象都获得？
    实际情况是：装饰器真正执行，是在他所装饰的函数名/类名被导入(from..import)时才会触发装饰器执行，我之前
    犯错是以为注册器会在一个@被执行后获得所有注册对象，结果发现一个@只能注册一个对象，所以只好在__init__中
    事先from..import了所有module，也就相当与导入了这些对象，也就触发了@才实现了所有对象的注册
