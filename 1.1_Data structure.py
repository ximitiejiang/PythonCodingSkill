...
Q4. 如何进行堆叠和展平？
核心概念：
    1. concatenate, stack只能输入array，不能输入list，而torch.cat可以输入list/array(其实是因为torch把list内部转化成array，所以其实都是适用于array)
...
b2 = b1[...,1][b1[...,1] > 2]  # 筛选 - 元素筛选式
np.where(b1>2)                 # 筛选 - 返回index list
b1 > 2                         # 筛选 - 返回bool list
